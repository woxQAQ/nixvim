...
let
  ...
  wrapBuilder =
    ...
    in
    if
      (builtins.hasAttr "name" arg && builtins.match "neotest" arg.name != null)
...
in
{
  ...
  neotest = prev.neotest.overrideAttrs (_: {
    doCheck = false;
  });

  luaPackages = luaPackages // {
    neotest = luaPackages.neotest.overrideAttrs (_: {
      doCheck = false;
    });
  };

  luajitPackages = luajitPackages // {
    neotest = luajitPackages.neotest.overrideAttrs (_: {
      doCheck = false;
    });
  };
  ...
  luajit_2_1 = nixpkgs-packages.luajit_2_1 // {
    neotest = nixpkgs-packages.luajit_2_1.neotest.overrideAttrs (_: {
      doCheck = false;
    });
  };
